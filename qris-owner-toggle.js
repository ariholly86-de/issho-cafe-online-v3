...
(function(){
  ...
  function ensureUI(){
    ...
    if(!box||!d.body.contains(box)){
      ...
      button.type='button';
      button.style.cssText='width:auto;min-width:0;border:0;border-radius:8px;padding:7px 11px;font-weight:400;color:#fff;cursor:pointer;font-size:12px;line-height:1.2;white-space:nowrap';
      button.textContent='💳 QRIS: MEMUAT...';
      status=d.createElement('div');
      status.style.cssText='font:400 10px Arial;color:#bbb;text-align:left;line-height:1.2';
  ...
  function sync(){normalizeTypography();if(!ensureUI())return;if(logged())read()}
