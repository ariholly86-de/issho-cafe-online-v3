...
public class BridgeServer {
  ...
  void handle(Socket s){try{InputStream in=s.getInputStream(); ByteArrayOutputStream h=new ByteArrayOutputStream(); int prev=-1,b; while((b=in.read())!=-1){h.write(b); if(prev=='\r'&&b=='\n'){String hs=new String(h.toByteArray(),StandardCharsets.ISO_8859_1); if(hs.endsWith("\r\n\r\n"))break;} prev=b;} String headers=new String(h.toByteArray(),StandardCharsets.ISO_8859_1); String first=headers.split("\r\n")[0]; if(first.startsWith("OPTIONS ")){send(s,204,"");return;} int len=0; for(String line:headers.split("\r\n")){if(line.toLowerCase().startsWith("content-length:"))len=Integer.parseInt(line.substring(line.indexOf(':')+1).trim());} byte[] body=new byte[len]; int off=0; while(off<len){int n=in.read(body,off,len-off);if(n<0)break;off+=n;} String path=first.split(" ")[1];
      if(path.startsWith("/status")){send(s,200,"{\"ok\":true,\"printer\":\"WP58D\",\"bridge\":\"ISSHO Printer Bridge\"}");}
      else if(path.startsWith("/connect")){try{Printer.connect(a,"WP58D");send(s,200,"{\"ok\":true,\"connected\":true,\"printer\":\"WP58D\"}");}catch(Exception e){send(s,500,"{\"ok\":false,\"error\":"+json(e.getMessage())+"}");}}
  ...
  void send(Socket s,int code,String body)throws IOException{byte[] b=body.getBytes(StandardCharsets.UTF_8);String h="HTTP/1.1 "+(code==200?"200 OK":code==204?"204 No Content":"500 ERROR")+"\r\nContent-Type: application/json; charset=utf-8\r\nAccess-Control-Allow-Origin: *\r\nAccess-Control-Allow-Methods: GET,POST,OPTIONS\r\nAccess-Control-Allow-Headers: Content-Type\r\nAccess-Control-Allow-Private-Network: true\r\nContent-Length: "+b.length+"\r\nConnection: close\r\n\r\n";s.getOutputStream().write(h.getBytes(StandardCharsets.ISO_8859_1));s.getOutputStream().write(b);s.getOutputStream().flush();}
}
