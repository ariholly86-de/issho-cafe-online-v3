...
public class MainActivity extends Activity {
    ...
    private BluetoothAdapter adapter; private BluetoothSocket socket; private OutputStream out; private BridgeServer server; private WebView web; private TextView nativeStatus; private TextToSpeech tts; private volatile boolean ttsReady=false;

    @Override public void onCreate(Bundle b){super.onCreate(b);adapter=BluetoothAdapter.getDefaultAdapter();initAlarm();buildApp();server=new BridgeServer();server.start();if(android.os.Build.VERSION.SDK_INT>=31&&checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT)!=PackageManager.PERMISSION_GRANTED)requestPermissions(new String[]{Manifest.permission.BLUETOOTH_CONNECT},REQ_BT);else startPrinterAutoConnect();}

    private void initAlarm(){tts=new TextToSpeech(this,status->{if(status==TextToSpeech.SUCCESS){ttsReady=true;tts.setLanguage(new Locale("id","ID"));tts.setSpeechRate(1.0f);tts.setPitch(1.0f);}});}

    private void buildApp(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(Color.BLACK);nativeStatus=new TextView(this);nativeStatus.setText("ISSHO CAFE • Dapur + Printer RPP02N: memeriksa koneksi…");nativeStatus.setTextColor(Color.WHITE);nativeStatus.setTextSize(13);nativeStatus.setPadding(16,12,16,12);root.addView(nativeStatus,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));web=new WebView(this);WebSettings s=web.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setAllowFileAccess(false);s.setMediaPlaybackRequiresUserGesture(false);if(android.os.Build.VERSION.SDK_INT>=21)s.setMixedContentMode(WebSettings.MIXED_CONTENT_ALWAYS_ALLOW);web.addJavascriptInterface(new AlarmBridge(),"AndroidDapurAlarm");web.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);installAlarmBridge();}});web.setWebChromeClient(new WebChromeClient());web.setOverScrollMode(WebView.OVER_SCROLL_NEVER);web.loadUrl(DAPUR_URL);root.addView(web,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1f));setContentView(root);}

    private void installAlarmBridge(){if(web==null)return;web.evaluateJavascript("(function(){try{if(window.__isshoAlarmNative)return;window.__isshoAlarmNative=true;var old=window.speechSynthesis&&window.speechSynthesis.speak;window.speechSynthesis.speak=function(u){try{var text=(u&&u.text)||'Perhatian. Order baru masuk.';if(window.AndroidDapurAlarm)window.AndroidDapurAlarm.speak(text);}catch(e){if(old)old.call(window.speechSynthesis,u);}};}catch(e){}})();",null);}

    private class AlarmBridge { @JavascriptInterface public void speak(String text){final String msg=(text==null||text.trim().isEmpty())?"Perhatian. Order baru masuk.":text;runOnUiThread(()->{if(ttsReady&&tts!=null)tts.speak(msg,TextToSpeech.QUEUE_FLUSH,null,"issho-alarm");});} }
    ...
    @Override public void onRequestPermissionsResult(int r,String[]p,int[]g){super.onRequestPermissionsResult(r,p,g);if(r==REQ_BT&&g.length>0&&g[0]==PackageManager.PERMISSION_GRANTED)startPrinterAutoConnect();else ui("Izin Nearby devices diperlukan untuk printer RPP02N. Alarm Dapur tetap aktif.");}
    @Override public void onBackPressed(){if(web!=null&&web.canGoBack())web.goBack();else super.onBackPressed();}
    @Override protected void onDestroy(){if(server!=null)server.stopServer();closePrinter();if(tts!=null){tts.stop();tts.shutdown();}if(web!=null)web.destroy();super.onDestroy();}
    ...
    private class BridgeServer extends Thread{private volatile boolean running=true;private ServerSocket ss;public void run(){try{ss=new ServerSocket(PORT,20,java.net.InetAddress.getByName("127.0.0.1"));while(running){final Socket s=ss.accept();Executors.newSingleThreadExecutor().execute(()->handle(s));}}catch(Exception ignored){}}void stopServer(){running=false;try{if(ss!=null)ss.close();}catch(Exception ignored){}}
        private void handle(Socket s){try{s.setSoTimeout(8000);InputStream in=s.getInputStream();String h=readHeaders(in);String[]first=h.split("\\r?\\n",2);String[]req=first[0].split(" ");String method=req[0],path=req[1];int len=headerInt(h,"Content-Length");byte[]body=readBody(in,len);if(method.equals("OPTIONS")){respond(s,200,"{\"ok\":true}");return;}if(path.equals("/status")&&method.equals("GET")){boolean c=socket!=null&&socket.isConnected()&&out!=null;respond(s,200,"{\"ok\":true,\"bridge\":true,\"connected\":"+c+",\"printer\":\"RPP02N\",\"model\":\"58mm Thermal\",\"port\":9100}");}else if(path.equals("/connect")&&method.equals("POST")){try{connectIfNeeded();ui("✓ RPP02N tersambung • Dapur siap cetak • Alarm aktif");respond(s,200,"{\"ok\":true,\"connected\":true,\"printer\":\"RPP02N\"}");}catch(Exception e){respond(s,500,"{\"ok\":false,\"error\":"+json(e.getMessage())+"}");}}else if(path.equals("/test")&&method.equals("POST")){try{send(testBytes());ui("✓ Test print RPP02N berhasil • Alarm aktif");respond(s,200,"{\"ok\":true,\"printed\":true}");}catch(Exception e){respond(s,500,"{\"ok\":false,\"error\":"+json(e.getMessage())+"}");}}else if(path.equals("/print")&&method.equals("POST")){try{if(body.length==0)throw new IOException("Data cetak kosong.");send(body);respond(s,200,"{\"ok\":true,\"printed\":true,\"bytes\":"+body.length+"}");}catch(Exception e){respond(s,500,"{\"ok\":false,\"error\":"+json(e.getMessage())+"}");}}else respond(s,404,"{\"ok\":false,\"error\":\"Endpoint tidak ditemukan.\"}");}catch(Exception ignored){}finally{try{s.close();}catch(Exception ignored){}}}
