...
function hideOldPrint(){
  ...
  try{
    ...
    reports.querySelectorAll('button').forEach(b=>{
      // JANGAN sembunyikan tombol Print milik Rekapan Detail.
      if(b.id==='mr-print'){
...
function printDetail(){
  ...
  if(!result||!result.innerHTML.trim()){
    alert('Silakan pilih Harian, Mingguan, atau Bulanan terlebih dahulu agar rekapan detail tampil.');
    return;
  }
...
function boot(){
  ...
  if(pr){pr.style.display='inline-flex';pr.style.visibility='visible';pr.style.opacity='1';}
  removeFavorite();
}
