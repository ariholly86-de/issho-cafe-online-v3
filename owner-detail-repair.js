...
function removeFavorite(){
  ...
  try{
    ...
    nodes.forEach(el=>{
      ...
      if(!target||target.id==='reports'||target.id==='reportBox'){
        target=el.parentElement;
      }
      if(target&&target!==d.body&&target.id!=='reports'&&target.id!=='reportBox'){
        target.remove();
      }else{
        el.remove();
      }
...
const a=d.getElementById('mr-daily'),b=d.getElementById('mr-weekly'),c=d.getElementById('mr-monthly');if(a)a.onclick=()=>run('daily');if(b)b.onclick=()=>run('weekly');if(c)c.onclick=()=>run('monthly');removeFavorite();}
const f=F();if(f)f.addEventListener('load',()=>setTimeout(boot,100));boot();setInterval(boot,1000);setInterval(removeFavorite,500);
})();/* MASTER OWNER REKAP LOCK — MENU FAVORIT REMOVED */