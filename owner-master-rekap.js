...
function installOwnerTools(){
 ...
 const parentBar=parentDoc.getElementById('bar');
 if(parentBar) parentBar.style.display='none';
 const d=D();
 if(!d)return;
 const app=d.getElementById('app');
 if(!app)return;
 const logged=!app.classList.contains('hidden') && getComputedStyle(app).display!=='none';
 let bar=d.getElementById('owner-tools-after-login');
 if(!logged){if(bar)bar.remove();return;}
 if(!bar){
  bar=d.createElement('div');
  bar.id='owner-tools-after-login';
  bar.style.cssText='display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin:0 0 12px;padding:10px;background:#151515;border:1px solid #444;border-radius:12px;position:relative;z-index:20;box-shadow:0 6px 18px rgba(0,0,0,.35)';
  bar.innerHTML='<button id="owner-restore-tool" type="button" style="border:0;border-radius:9px;padding:10px 13px;background:#2f7d50;color:#fff;font-weight:900;cursor:pointer">🟢 PULIHKAN MENU TERPILIH (0)</button><button id="owner-manager-tool" type="button" style="border:0;border-radius:9px;padding:10px 13px;background:#315b9a;color:#fff;font-weight:900;cursor:pointer">🍽️ Kelola Menu & Kategori</button><button id="owner-add-tool" type="button" style="border:0;border-radius:9px;padding:10px 13px;background:#b68142;color:#fff;font-weight:900;cursor:pointer">➕ Tambah Menu Baru</button>';
  app.insertBefore(bar,app.firstElementChild||null);
  const r=bar.querySelector('#owner-restore-tool'),m=bar.querySelector('#owner-manager-tool'),a=bar.querySelector('#owner-add-tool');
  if(r)r.onclick=()=>{try{parentDoc.getElementById('restore')?.click()}catch(e){}};
  if(m)m.onclick=()=>{window.top.location.href='owner-menu-manager.html?v=20260905-owner-manager'};
  if(a)a.onclick=()=>{window.top.location.href='owner-add-menu.html?v=20260905-owner-add'};
 }
 const parentRestore=parentDoc.getElementById('restore'),target=d.getElementById('owner-restore-tool');
 if(target&&parentRestore)target.textContent=parentRestore.textContent;
}
function boot(){
 installOwnerTools();
 const d=D(),w=W();if(!d||!w||!d.getElementById('reports'))return;const rb=d.getElementById('reportBox');if(!rb)return;let box=d.getElementById('master-rekap-detail');if(!box){box=d.createElement('section');box.id='master-rekap-detail';box.style.cssText='display:block!important;visibility:visible!important;margin-top:16px;padding:16px;background:#151515;border:1px solid #444;border-radius:15px;color:#f3f3f3';box.innerHTML='<h2 style="margin:0 0 6px">📊 Rekapan Detail Sesuai Kategori</h2><div style="color:#aaa;font-size:13px;margin-bottom:12px">Laporan detail penjualan berdasarkan kategori dan menu.</div><div style="display:flex;gap:8px;flex-wrap:wrap"><button id="mr-daily" type="button" style="border:0;border-radius:9px;padding:11px 14px;background:#b68142;color:#fff;font-weight:900;cursor:pointer">📅 Harian</button><button id="mr-weekly" type="button" style="border:0;border-radius:9px;padding:11px 14px;background:#315b9a;color:#fff;font-weight:900;cursor:pointer">📆 Mingguan</button><button id="mr-monthly" type="button" style="border:0;border-radius:9px;padding:11px 14px;background:#2f7d50;color:#fff;font-weight:900;cursor:pointer">🗓️ Bulanan</button></div><div id="mr-result" style="margin-top:12px"></div>';rb.parentNode.insertBefore(box,rb.nextSibling)}else{box.style.display='block';box.style.visibility='visible'}
...
const a=d.getElementById('mr-daily'),b=d.getElementById('mr-weekly'),c=d.getElementById('mr-monthly');if(a)a.onclick=()=>run('daily');if(b)b.onclick=()=>run('weekly');if(c)c.onclick=()=>run('monthly');}
const f=F();if(f)f.addEventListener('load',()=>setTimeout(boot,100));boot();setInterval(boot,1000);
})();/* MASTER OWNER REKAP + TOOLS DI DALAM SETELAH LOGIN */